...
cd saga && ./gradlew test --tests "com.saga.orchestrator.SagaOrchestratorTest"
...
Compose service is still named `bff` (matches `BffApplication`/`CheckoutController`), only the
directory is `saga/`.

## Architecture

`saga/src/main/java/com/saga/` has one package per former microservice — `orders/`, `inventory/`,
`payments/`, `loyalty/`, `shipping/` — as **siblings** of `bff/` (not nested inside it), each
with its own `domain/application/infrastructure/persistence/entity`. Same hexagonal shape as the
microservices on `main`, just called via direct Java method calls instead of NATS, sharing one
MariaDB database (`monolith`) instead of one-per-domain. `orchestrator/` is a sixth, cross-cutting
package — not a domain, it's the generic saga engine shared by all of them.

- `orchestrator/SagaStep.java` — the contract every checkout step implements: `execute()` and
  `compensate()`, no generics, no return type. Each implementation is a small **per-request
...
- `orchestrator/SagaOrchestrator.java` — the actual saga machinery, read this file first when
  working on failure/compensation behavior. `run(List<SagaStep>)` executes steps in order,
...
- `bff/application/CheckoutUseCase.java` — defines *this* saga: builds the 6 steps in order
  (`CreateOrderStep`, `ReserveStockStep`, `ChargePaymentStep`, `GrantLoyaltyPointsStep`,
  `GenerateShippingStep`, `ConfirmOrderStep`) fresh on every call (they carry per-request state)
  and hands the list to `SagaOrchestrator.run(...)`. `ConfirmOrderStep` is the one step that
  depends on another step's result (the `Order` `CreateOrderStep` created) — it takes that step
  directly in its constructor rather than through a shared context object; nothing else in this
  saga has a cross-step dependency, so don't introduce a context object for a problem that
  doesn't exist yet.
- `bff/web/CheckoutController.java` — `POST /checkout` is synchronous: it returns the final
...
- `bff/web/CheckoutExceptionHandler.java` — maps the three domain failure exceptions
...
- Each domain's `application/*Service` exposes both the forward action and its compensation as
  ...
  `PaymentService.charge`/`refund`, `LoyaltyService.grant`/`revert`,
  `ShippingService.generate`/`cancel`) — no envelope, no command type string, no NATS subject.
  The `*Step` classes call these directly and hold the returned domain object (`Order`,
  `Payment`, `LoyaltyGrant`, `Shipment`) in a field so `compensate()` has what it needs (id,
  current state).
- Each domain's `infrastructure/persistence/entity/` holds its JPA `@Entity`; repository ports
  ...
  Adapters that support compensation (payments, loyalty, shipping — orders/inventory already had
  this) follow the same find-by-id-and-mutate-the-managed-entity pattern used everywhere else in
  this codebase — **never** build a fresh detached entity and `save()` it to update, always
  `findById` inside the same call and mutate that managed instance (see any `*RepositoryAdapter`
  for the pattern; this matters for JPA merge semantics, not just style).
